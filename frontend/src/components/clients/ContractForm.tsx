...
export function ContractForm({ initial, onSave, onCancel }: ContractFormProps) {
	...
	const [reminderPopup1h, setReminderPopup1h] = useState(() => {
		const reminders = initial?.calendar_reminders;
		return reminders
			? reminders.some((row) => row.method === "popup" && row.minutes === 60)
			: true;
	});
	...
	const weekdays = (
		t("common.weekdays.short", { returnObjects: true }) as string[]
	).map((label, i) => ({
		index: String(i),
		label,
	}));
	...
	const handleSubmit = async (e: FormEvent) => {
		...
		if (invalidDay) {
			const label =
				weekdays.find((weekday) => weekday.index === invalidDay[0])?.label ?? invalidDay[0];
		...
		try {
			await onSave({
				...
				schedule_days:
					Object.keys(scheduleDays).length > 0 ? scheduleDays : null,
								...
								return (
									<div key={index} className="flex items-center gap-2">
										...
										<input
											...
											value={day.start}
											onChange={(e) =>
												setDayTime(index, "start", e.target.value)
											}
			...
			<div className="space-y-3 pt-1 border-t border-slate-100">
				<div className="flex items-center justify-between">
					...
					>
						<input
							...
							checked={form.notify}
							onChange={(e) =>
								setForm((prev) => ({ ...prev, notify: e.target.checked }))
							}
							className="w-4 h-4 accent-primary"
						/>
				...
				<div>
					...
					<textarea
						...
						onChange={(e) =>
							setForm((prev) => ({ ...prev, calendar_description: e.target.value }))
						}
						rows={3}
					...
					<div className="space-y-1.5">
						...
						>
							...
							<span className="text-sm text-slate-700">
								{t("contracts.form.reminderEmail24h")}
							</span>
						</label>
						<label
						...
						>
							...
							/>
							<span className="text-sm text-slate-700">
								{t("contracts.form.reminderPopup1h")}
							</span>
						</label>
					</div>
				</div>
