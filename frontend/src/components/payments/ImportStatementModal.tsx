          ...
          {period.month && (
            <div className='flex flex-wrap items-center gap-2 px-3 py-2 rounded-lg bg-primary/5 border border-primary/15 text-sm'>
              ...
              <span className='text-slate-700'>
                <strong>
                  {t('import.periodLabel', { month: months[period.month - 1], year: period.year })}
                </strong>
                {' · '}
          ...
          {fileAlreadyImportedAt && (
            <div className='flex items-center gap-2 px-3 py-2 rounded-lg bg-amber-50 border border-amber-200 text-sm text-amber-800'>
              ...
              {t('import.alreadyImported', {
                date: new Date(fileAlreadyImportedAt).toLocaleDateString('es-ES'),
              })}
            </div>
          )}
                  ...
                  <tr key={i} className={row.skip ? 'opacity-40' : ''}>
                      ...
                      <select
                        ...
                        onChange={(e) =>
                          setRows((prev) =>
                            prev.map((r, j) =>
                              j === i
                                ? { ...r, selected_client_id: parseInt(e.target.value, 10) || null }
                                : r,
                            ),
                          )
                        }
                    ...
                    <td className='px-4 py-3 text-center'>
                      <input
                        ...
                        onChange={(e) =>
                          setRows((prev) =>
                            prev.map((r, j) => (j === i ? { ...r, skip: !e.target.checked } : r)),
                          )
                        }
